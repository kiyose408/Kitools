...
### Bug修复
- **B站分类错误修复**
  - 修复访问B站时被错误分类到"其他"的问题
  - 增强窗口标题识别：支持"哔哩哔哩"、"B站"、"bilibili"、"b23.tv"等多种格式
  - 添加特殊匹配逻辑，确保B站视频正确分类到"视频娱乐"

- **抖音分类优化**
  - 支持"抖音"、"douyin"、"tiktok"等多种标识识别
  - 确保抖音视频正确分类到"视频娱乐"

### 改进
- **调试输出优化**
  - 大幅减少冗余日志输出：从每秒输出改为仅在窗口切换时输出
  - 单次日志输出从5-6行减少到2-3行
  - 窗口标题截断显示（前50字符），避免过长标题影响可读性
  - 添加浏览器分类详细调试信息，方便排查分类问题
...
### 技术细节
- activitymanager.cpp:423-462: 增强categorizeBrowser()函数
- windowmonitor.cpp:82-118: 优化onTimerTick()调试输出
- 添加浏览器分类调试日志（匹配路径追踪）
