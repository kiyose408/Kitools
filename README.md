...
| 功能 | 描述 |
|------|------|
| 全盘索引 | 自动扫描开始菜单、桌面、Program Files等目录 |
| 分类筛选 | 按类型查看（全部/常用/最近/固定/系统/开发/办公/网络/媒体） |
| 快速搜索 | 按名称和关键词搜索应用 |
| 使用统计 | 记录启动次数，智能排序 |
| 固定功能 | 固定常用应用到顶部 |
...
│
│       └── launcher/                # 快捷启动器模块
│           ├── launchercontroller.h/cpp
│           ├── launcherindexer.h/cpp
│           ├── launchersettingspanel.h/cpp
│           └── launcherdatatypes.h
│
├── CMakeLists.txt                   # 构建配置
└── README.md
...
| ~~快捷启动器~~ | ~~快速启动应用、文件、网址~~ ✅ 已完成 |
| 全局快捷键 | Alt+Space快速呼出启动器 |
